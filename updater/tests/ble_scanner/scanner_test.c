...
bool dfu_runner_cancelled(void) { return run_cancelled; }
...
void fake_event(const char *event)
{
	...
	if (send_match && !strcmp(event, "wait")) {
		send_match = false;
		uint8_t uuid[] = {0x59, 0xfe};
		struct bt_data field = {BT_DATA_UUID16_ALL, sizeof(uuid), uuid};
...
int main(int argc, char **argv)
{
	...
	if (argc > 1) {
		reset_test();
		cancel_at = "init";
		assert(ble_scanner_find_first(&out, 0, NULL, -127, NULL) == -ECANCELED);
	...
	reset_test();
	stop_run();
	assert(ble_scanner_find_pinned(&out, 0, &peer) == -ECANCELED);
	assert(ble_scanner_seen_at(&peer, 100, &out) == -ECANCELED);
	assert(starts == 0);
