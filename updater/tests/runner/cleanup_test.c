...
static enum dfu_result resource_run(const struct dfu_target *t,
  const struct dfu_payload *p, const struct app_config *cfg, bool (*cancelled)(void))
{
  (void)p; (void)cfg;
  assert(cancelled && !cancelled());
  assert(associated && t->ble.rssi == (int8_t)finds);
  runs++;
  if (scenario == CANCEL_REFIND) return DFU_BUTTONLESS_TRIGGERED;
...
static const struct dfu_transport resource_transport = {
  .name = "resource-owning", .find = resource_find,
