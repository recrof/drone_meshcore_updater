				       ...
				       bt_gatt_discover_params *params)
{
	...
	if (self == nullptr || conn != self->conn_ || params != &self->disc_params_) {
	...
	const bt_uuid *wanted = self->secure_ ? &kSecureServiceUuid.uuid : &kServiceUuid.uuid;
				    ...
				    bt_gatt_discover_params *params)
{
	...
	const bt_uuid *control = self->secure_ ? &kSecureControlUuid.uuid : &kControlPointUuid.uuid;
	const bt_uuid *packet = self->secure_ ? &kSecurePacketUuid.uuid : &kPacketUuid.uuid;
			  ...
			  uint16_t length)
{
	...
	if (self == nullptr || conn != self->conn_ || params != &self->read_params_) {
		return BT_GATT_ITER_STOP;
	}
	self->att_err_ = err;
...
void GattLink::mtu_cb(bt_conn *conn, uint8_t err, bt_gatt_exchange_params *params)
{
	...
	if (self == nullptr || conn != self->conn_ || params != &self->mtu_params_) {
		return;
	}
	self->att_err_ = err;
	k_sem_give(&self->op_sem_);
