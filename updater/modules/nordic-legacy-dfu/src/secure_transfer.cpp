...
Report SecureTransfer::run(const Firmware &firmware)
{
	...
	/* Never append to an unrelated init command or unverified image prefix.
	 * Refuse instead of silently discarding an interrupted different update. */
	...
	if (!offset && !create(1, init.size())) return report_;
	...
	if (offset && (offset % maximum == 0 || offset == image.size())) {
		if (!execute()) {
	...
	while (offset < image.size()) {
		...
		if (offset % maximum == 0 && !create(2, remaining)) return report_;
